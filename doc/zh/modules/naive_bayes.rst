...
分布参数由每类 :math:`y 的 :math:`\theta_y = (\theta_{y1},\ldots,\theta_{yn})`向量决定， 式中 :math:`n` 是特征的数量（对于文本分类，是词汇量的大小）
:math:`\theta_{yi}` 是样本中属于类 :math:`y` 中特征 :math:`i` 概率 :math:`P(x_i \mid y)` 。
