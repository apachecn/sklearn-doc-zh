...
*   **Metric functions（指标函数）**: `metrics` 模块实现了针对特定目的评估预测误差的函数。这些指标在以下部分部分详细介绍 [分类指标](#_332-分类指标), [多标签排名指标](#_333-多标签排名指标), [回归指标](#_334-回归指标) 和 [聚类指标](#_335-聚类指标) 。
...
`jaccard_score`就像`precision_recall_fscore_support`中的设定方法，本身应用于二分类，并通过使用从二分类扩展到多标记和多类（见[从二分到多分类和multilabel](#_3321-从二分到多分类和-multilabel)）。
